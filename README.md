...
| カレントフォルダーが初期値(D) | ON で検索場所の初期値を編集中ファイルのフォルダーにする（後述） |
...
既定は **前回検索したフォルダー**。マクロから開いても、編集中ファイルのフォルダーで
上書きされることはない。

「カレントフォルダーが初期値(D)」を ON にすると、サクラエディタと同じく
**編集中ファイルのフォルダー**が毎回の初期値になる。

どちらの設定でも **[現フォルダー(G)]** ボタン（`Alt+G`）で編集中ファイルのフォルダーへ
すぐ移動できる。
...
結果を開くと、検索語に**検索マーク（ハイライト）**が付く。色はサクラエディタの
タイプ別設定『カラー』の「検索文字列」で決まる。

起動時マクロ（`-M` / `-MTYPE=js`）で `SearchNext` → `SearchClearMark` を実行している。
検索語は `String.fromCharCode()` で組み立てて渡すので、引用符や空白を含んでいても壊れない。

- ユーザーの検索履歴と検索オプションは変更しない（マクロ専用の bit 12 / 13 を使用）
- **正規表現で検索したときは付けない。** サクラエディタとは正規表現の方言が違うため
- 単語単位検索のときは、内部で使う正規表現ではなく**元の語**で付ける
- 検索は結果の先頭行から下方向に走るので、ヘッダー行はヒットしない
